import { ColorTheme } from '@acme/types';

export const USER_DEFAULT_THEME: ColorTheme = {
  radius: '0.625rem',

  background: 'oklch(0.99 0.03 30)',
  foreground: 'oklch(0.15 0.02 30)',

  card: 'oklch(1 0 0)',
  cardForeground: 'oklch(0.15 0.02 30)',

  popover: 'oklch(1 0 0)',
  popoverForeground: 'oklch(0.15 0.02 30)',

  primary: 'oklch(0.55 0.16 150)',
  primaryForeground: 'oklch(0.98 0.01 150)',

  secondary: 'oklch(0.92 0.05 150)',
  secondaryForeground: 'oklch(0.18 0.04 150)',

  muted: 'oklch(0.95 0.02 150)',
  mutedForeground: 'oklch(0.45 0.03 150)',

  accent: 'oklch(0.9 0.06 150)',
  accentForeground: 'oklch(0.18 0.04 150)',

  destructive: 'oklch(0.58 0.24 27)',

  border: 'oklch(0.9 0.02 150)',
  input: 'oklch(0.9 0.02 150)',
  ring: 'oklch(0.55 0.16 150)',

  sidebar: 'oklch(0.97 0.02 150)',
  sidebarForeground: 'oklch(0.15 0.02 150)',
  sidebarPrimary: 'oklch(0.55 0.16 150)',
  sidebarPrimaryForeground: 'oklch(0.98 0.01 150)',
  sidebarAccent: 'oklch(0.9 0.05 150)',
  sidebarAccentForeground: 'oklch(0.18 0.04 150)',
  sidebarBorder: 'oklch(0.88 0.02 150)',
  sidebarRing: 'oklch(0.55 0.16 150)',
};

export const USER_DARK_THEME: ColorTheme = {
  ...USER_DEFAULT_THEME,
  background: 'oklch(0.15 0.02 150)',
  foreground: 'oklch(0.99 0.03 150)',
  card: 'oklch(0.2 0.03 150)',
  cardForeground: 'oklch(0.99 0.03 150)',
  popover: 'oklch(0.2 0.03 150)',
  popoverForeground: 'oklch(0.99 0.03 150)',
  primary: 'oklch(0.75 0.25 150)',
  primaryForeground: 'oklch(0.98 0.01 150)',
  secondary: 'oklch(0.25 0.08 150)',
  secondaryForeground: 'oklch(0.98 0.01 150)',
  muted: 'oklch(0.22 0.03 150)',
  mutedForeground: 'oklch(0.75 0.02 150)',
  accent: 'oklch(0.3 0.12 150)',
  accentForeground: 'oklch(0.98 0.01 150)',
  destructive: 'oklch(0.75 0.35 27)',
  border: 'oklch(0.3 0.03 150)',
  input: 'oklch(0.3 0.03 150)',
  ring: 'oklch(0.75 0.25 150)',
  sidebar: 'oklch(0.22 0.03 150)',
  sidebarForeground: 'oklch(0.99 0.03 150)',
  sidebarPrimary: 'oklch(0.75 0.25 150)',
  sidebarPrimaryForeground: 'oklch(0.98 0.01 150)',
  sidebarAccent: 'oklch(0.3 0.12 150)',
  sidebarAccentForeground: 'oklch(0.98 0.01 150)',
  sidebarBorder: 'oklch(0.35 0.02 150)',
  sidebarRing: 'oklch(0.75 0.25 150)',
};
